...
function toChange(
    ...
    $.each(change, function(index, coin) {
...
input.keyup(function(e) {
    ...
    try {
        ...
        calculate.attr('disabled', '');
    }
});
calculate.click(function(e) {
    var total = 0;
    $.each(toChange(amount), function(index, coin) {
        total += coin.numberOfCoins;
        output.html(output.html() + `${coin.toStr}ｘ${coin.numberOfCoins}<br />`);
    });
    output.html(output.html() + `The total amount of coins is <b><i>${total}</i></b><br />`);
